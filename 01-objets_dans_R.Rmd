...
#### Vecteur d'une variable qualitative
Les variables qualitatives sont de type `factor` (pour des variables qualitatives nominales) ou de type `ordered` (pour les variables qualitatives ordinales).
...
Les fonctions `factor()` et `as.factor()` permettent de définir une variable qualitative nominale. De même, les fonctions `ordered()` et `as.ordered()` permettent de définir une variable qualitative ordinale.

Les objets de type `factor` (ou `ordered`) ont une liste d'**attributs** que l'on peut obtenir par la fonction `attributes()`. 
Par défaut, les fonctions `factor()` et `ordered()` vont créer des `levels`, correspondant aux catégories de la variable qualitative. Pour cela, il va utiliser chaque valeur unique, les ranger de la plus petite à la plus grande, et transformer ces valeurs en chaînes de caractères.
```{r factor1, eval=FALSE, include=TRUE}
# On commence avec la variable saisie sous forme de vecteur d'entiers
var_1 <- c(0, 9, 0, 0, 1, 9, 1, 0, 0, 9, 1, 9, 0, 1, 1)

# vous pouvez définir ce vecteur numérique en "factor"
var_quali <- as.factor(var_1)  # autre possibilité : var.quali <- factor(var_1)
print(var_quali)
class(var_quali)

# les fonction "is.factor" ou "is.ordered" permettent de vérifier si le type de 
# l'objet est bien une variable qualitative nominale ("factor") ou ordinale 
# ("ordered")
is.factor(var_1) # FALSE, l'objet "var_1" est un vecteur numérique
is.factor(var_quali) # TRUE, l'objet "var.quali" est bien de type "factor"

# La variable var.quali a des attributs, que l'on va enregistre dans un nouvel 
# objet "attr_var.quali"
attr_var_quali <- attributes(var_quali)
class(attr_var_quali) # les attributs sont un objet de type "list"
attr_var_quali
# la liste contient deux sous-objets "levels" et "class". 
# On peut obtenir les sous-objets de la liste avec l'opérateur dollar "$"
attr_var_quali$levels # "0" "1" "9"
attr_var_quali$class # factor

class(attr_var_quali$levels) 
# les levels sont définis comme une chaîne de caractères, rangés du plus petit
# au plus grand 
```

Alternativement, on peut attribuer des étiquettes `labels = c("non", "oui", "ne sait pas")` à la place des `levels = c("0","1","9")`. Attention, les étiquettes doivent être indiquées dans le même ordre que les `levels` (du code le plus petit au plus grand) (cf. `?factor`). 

Si vous souhaitez retransformer la variable en format numérique, vous pouvez appliquer la fonction `as.numeric` au facteur. Toutefois, une nouvelle numérotation par défaut sera appliquée en commançant par la valeur 1. 
```{r factor2, eval=FALSE, include=TRUE}
# On reprend la variable précédente (avec de possibles valeurs manquantes NA)
var_1 <- c(0, 2, 0, 0, 1, 2, NA, 0, NA, 2, 1, 2, 0, 1, 1)

# on renseigne les arguments 
var_quali <- factor(var_1,
                    labels = c("non", "oui", "ne sait pas"))
var_quali
# si l'on demande d'imprimer la variable, les valeurs apparaîssent à présent
# sous leurs étiquettes 

attr_var_quali <- attributes(var_quali)
attr_var_quali$levels
# les levels sont à présent le vecteur de caractère c("non","oui","ne sait pas")

new_var_quali <- as.numeric(var_quali)
new_var_quali
# [1]  1  3  1  1  2  3 NA  1 NA  3  2  3  1  2  2

# si vous voulez que les manquants soient considérés comme une catégorie
# en tant que telle, on peut utiliser la fonction "addNA"
...
length(vect_int) # vecteur de longueur 5
length(vect_new) # vecteur de longueur 8
length(vect_char) # vecteur de longueur 6
length(vect_logic) # vecteur de longueur 8
length(var_quali) # vecteur de longueur 15
```

#### Nommer (`names()`) les éléments d'un vecteur
...
```{r namesvector, eval=FALSE, include=TRUE}
# on créer un vecteur de scores de satisfaction entre 1 et 5
# 1 = non satisfait
# 2 = un peu satisfait
