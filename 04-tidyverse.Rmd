...
# entre guillements)
```
```{r tidy20, eval=FALSE, include=TRUE}
...
### Description de variables quantitatives
...
_Note : pour appliquer une syntaxe en "pipe", la commande `fct_count()` prend pour 1er argument un vecteur de type "factor". Il faut donc utiliser la fonction `pull()` plutôt que `select()` pour sélectionner une variable et la retourner au format vecteur.
```{r tidy23, echo=TRUE}
